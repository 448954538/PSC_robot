...
END SUB
 ...
 'HMI界面按下软件触发按钮时响应的函数
GLOBAL SUB btn_softtrigger()  '扫描相机 
...
'HMI界面按下硬件触发按钮时响应的函数
GLOBAL SUB btn_line0trigger()         '扫描相机 
  CAM_SCAN("mvision")                 '扫描海康相机
  cam_num = CAM_COUNT()
  ...
  CAM_SEL(0)                        '选择相机id 
  CAM_SETMODE(1)                    '设置相机为硬件触发采集模式
  CAM_SETPARAM("TriggerActivation",1) '设置相机接收下降沿触发信号有效
  CAM_START(0)                      '开启相机
